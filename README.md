...
| 阶段 | 耗时 | 说明 |
|------|------|------|
| FFmpeg 抽帧 + CLAHE | ~1 min | 1809 帧 @ FPS=10 |
| COLMAP 特征提取 + 匹配 | ~13 min | 4590 SIFT 特征/帧 |
| COLMAP Mapper (CPU BA) | ~4-6 h | SIMPLE_RADIAL，~98% 注册率 |
| 3DGUT 训练 (30K iter, `-d 2`) | ~15 min | PSNR ~23.7, SSIM ~0.88 |
| 3DGUT 训练 (45K iter, `-d 2`) | ~22 min | PSNR ~24.1, SSIM ~0.88 |
| PPISP 蒸馏 + USDZ 导出 | ~3 min | 428 MB USDZ |

> **显存说明**: `-d 1` (全分辨率) 需要 >24 GB，RTX 4090 请使用默认 `-d 2`。
